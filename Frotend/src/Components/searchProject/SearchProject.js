...
function SearchProject() {
    ...
    const handleUserChange = (e) => {
        ...
        if (userId) {
            const userTodos = allTodos.filter(todo => todo.assignedUserId == userId);
            if (userTodos.length > 0) {
                const projectIds = [...new Set(userTodos.map(todo => todo.projectID))];
                const foundProjects = originalProjects.filter(project => projectIds.includes(project.id));
